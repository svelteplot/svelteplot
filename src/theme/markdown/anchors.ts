    ...
    return (tree, vFile) => {
        ...
        visit(tree, (node, idx, parent) => {
            if (node.type === 'heading' && !node.data?.anchorAdded) {
                ...
                const title = (node.children as any[])
                    .filter((c) => ['text', 'inlineCode', 'SvpHeading'].includes(c.type))
                    .map((c) => c.value)
                    .join('');
                let slugId = title.replace(/[&#$?* :/]/g, '-');
                while (anchors.some((an) => an.slugId === slugId)) slugId += '_';
