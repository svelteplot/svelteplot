...
const _highlighter: Highlighter = async (code, lang) => {
    ...
    if (shikiHighlighterInstance.twoslashTransformer) {
        shikiOptions.transformers?.push(shikiHighlighterInstance.twoslashTransformer);
    }
...
const highlighter: Highlighter = async (code, lang, meta) => {
    ...
    if (lines[0] === '// @noErrors') noErrorsFirstLine = lines.shift();
    code = lines
        .map((line, i) => {
    ...
    if (titleMeta) title = titleMeta.split('=')[1].replace(/(^")|("$)/g, '');
    if (noErrorsFirstLine) code = `${noErrorsFirstLine}\n${code}`;
...
`
  ...
  <div class="svp-code-block${containLineNumbers ? ' svp-code-block--with-line-numbers' : ''}">
    ...
    if (env.NODE_ENV === 'development') cache.set(cacheKey, cached);
    return cached;
};
    ...
    if (highlighterConfig.twoslash) {
        shikiHighlighterInstance.twoslashTransformer = createTransformerFactory(
            await createTwoslasher(
                highlighterConfig.twoslash === true
                    ? {
                          compilerOptions: {
                              module: 199,
                              moduleResolution: 99,
                              jsx: 1,
                              types: [
                                  '@sveltepress/vite/types',
                                  '@sveltepress/theme-default/types',
                                  '@sveltepress/theme-default/components',
                                  '@sveltejs/kit'
                              ]
                          }
                      }
