                ...
                if (textNode && textNode.type === 'text') {
                    ...
                    if (matches) {
                        ...
                        if (params) {
                            ...
                            const realPath = path.startsWith('.')
                                ? resolve(dir, path)
                            ...
                            if (existsSync(realPath)) {
                                ...
                                if (valueArr.length) {
                                    ...
                                    const initialBlankNumbers =
                                        firstLine.length - firstLine.trimStart().length;
                                    if (initialBlankNumbers > 0)
                                        valueArr = valueArr.map((line) =>
                                            line.replace(
                                                new RegExp(`^ {${initialBlankNumbers}}`),
                                                ''
                                            )
                                        );
                                }
                                ...
                                const node = {
                                    type: 'code',
                                    lang,
                                    value: codeStr.endsWith(',')
                                        ? codeStr.slice(0, codeStr.length - 1)
                                        : codeStr
                                };
