...
export const highlightLine: Command = (linesNumberToHighlight, idx, lines) => {
    ...
    if (Number.isNaN(num) || num < 1) return warpLine('svp-code-block--hl', idx);
    const max = lines - idx;
    return Array.from({ length: num > max ? max : num })
        .map((_, i) => {
...
export const focus: Command = (linesNumberToFocus, idx, lines) => {
    ...
    const res = [wrapFocus('0', `calc(12px + ${idx * 1.5}em)`)];
    res.push(
        wrapFocus(
            `calc(12px + ${(start + 1) * 1.5}em)`,
            `calc(12px + ${(lines - start - 1) * 1.5}em)`
        )
    );
...
export const processCommands: (
    ...
    while (matches && matches.length) {
        ...
        if (commandExecutor) commandDoms.push(commandExecutor(params, lineIndex, lineLength));
