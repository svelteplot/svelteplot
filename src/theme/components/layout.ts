...
export const resolvedSidebar = writable(
    Object.entries(themeOptions.sidebar || {}).reduce<LinkItem[]>(
        (all, [, item]) => [...all, ...item],
        []
    )
);
...
scrollY.subscribe((sy) => {
    ...
    if (nextDirection !== get(scrollDirection)) scrollDirection.set(nextDirection);
});

resolvedSidebar.subscribe((sidebar) => {
    pages.set(
        sidebar.reduce<LinkItem[]>(
            (allPages, item) =>
                Array.isArray(item.items) ? [...allPages, ...item.items] : [...allPages, item],
            []
        )
    );
});
...
sidebarCollapsed.subscribe((v) => {
    if (!v) tocCollapsed.set(true);
});

tocCollapsed.subscribe((v) => {
    if (!v) sidebarCollapsed.set(true);
});
