...
function asInterval([name, period]: [string, number], type: 'time' | 'utc') {
    let interval: TimeInterval | CountableTimeInterval | undefined = (
        type === 'time' ? timeIntervals : utcIntervals
    ).get(name);
    if (period > 1) {
        interval = (interval as CountableTimeInterval).every(period) ?? undefined;
...
export function generalizeTimeInterval(interval: TimeInterval, n: number) {
    if (!(n > 1)) return; // no need to generalize
    const duration: number | undefined = (interval as any)[intervalDuration];
    ...
    return ((interval as any)[intervalType] === 'time' ? maybeTimeInterval : maybeUtcInterval)(i);
}
