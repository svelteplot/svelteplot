    ...
    it('groups densities by fill and facet channels', () => {
        const input = {
            data: [
                ...
                { fx: 'A', fill: 'red', value: 1 },
                { fx: 'A', fill: 'blue', value: 0 },
                { fx: 'A', fill: 'blue', value: 1 },
                { fx: 'B', fill: 'red', value: 0 },
                { fx: 'B', fill: 'red', value: 1 },
                { fx: 'B', fill: 'blue', value: 0 },
                { fx: 'B', fill: 'blue', value: 1 }
            ],
            x: 'value',
            fill: 'fill',
            fx: 'fx'
        ...
        const { data, ...channels } = densityX(input, { bandwidth: 1, interval: 1, trim: true });
        ...
        for (const [, values] of grouped) {
            ...
            expect(sorted.map((d) => d.x)).toEqual([0, 1]);
            sorted.forEach((d) => expect(d.y).toBeCloseTo(0.375, 6));
        }
    });
});
