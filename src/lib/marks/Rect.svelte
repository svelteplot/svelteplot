        ...
        <GroupMultiple class={scaledData.length > 1 ? className : null} length={scaledData.length}>
            {#each scaledData as d, i (i)}
                {#if d.valid}
                    ...
                    {@const x1 = d.x1 == null ? plot.options.marginLeft + dx : d.x1}
                    {@const x2 =
                        d.x2 == null ? plot.options.marginLeft + plot.facetWidth + dx : d.x2}
                    {@const y1 = d.y1 == null ? plot.options.marginTop + dy : d.y1}
                    {@const y2 =
                        d.y2 == null ? plot.options.marginTop + plot.facetHeight + dy : d.y2}
