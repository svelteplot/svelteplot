    ...
    <div class="file-tabs">
        {#each workspace.files as File[] as file, index (file.name)}
            <div
                ...
                <span class="filename">
                    {(file === workspace.current && file.name !== 'App.svelte'
                        ? input_value
                        : file.name) + (workspace.modified[file.name] ? '*' : '') || ' '}
                </span>
        ...
        <Toolbox>
            ...
            <label class="option" aria-disabled={!workspace.supports_async}>
                ...
                <Checkbox
                    ...
                    onchange={() =>
                        workspace.update_compiler_options({
                            async: !workspace.compiler_options.async
                        })}></Checkbox>
            </label>

            <button disabled={!can_migrate} onclick={migrate}
                >Migrate to Svelte 5, if possible</button>
            ...
            <label class="option">
                ...
                <input
                    ...
                    placeholder="latest"
                    onchange={(ev) =>
                        workspace.set_svelte_version(ev.currentTarget.value || 'latest', true)} />
            </label>
