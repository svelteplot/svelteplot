    ...
    {#snippet dropdown()}
        <div class="popup">
            {#if Number.isInteger(majorVersion) && majorVersion < 5}
                <p>
                    <a href="/blog/runes">Runes</a> are available from Svelte 5 onwards, and this
                    playground is using Svelte {svelteVersion}.
                </p>
            ...
            {:else if workspace.current.name.endsWith('.js')}
                <p>
                    To use <a href="/blog/runes">runes</a> in a JavaScript file, change the
                    extension to
                    <code>.svelte.js</code>.
                </p>
            {:else if workspace.current.name.endsWith('.svelte')}
                ...
                {:else}
                    ...
                    <p>
                        To enable runes mode, either start using runes in your code, or add the
                        following to the top of your component:
                    </p>
                    <pre><code>&lt;svelte:options runes /&gt;</code></pre>
                {/if}
            {:else}
                <p>
                    Edit a <code>.svelte</code>, <code>.svelte.js</code> or <code>.js</code> file to
                    see information on <a href="/blog/runes">runes mode</a>
                </p>
            {/if}
        </div>
    {/snippet}
