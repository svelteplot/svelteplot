...
const takeScreenshot = async (page, urlPath, outputPath, isDarkMode = false) => {
  ...
  const elementHandle = await page.evaluateHandle(() => document.querySelector('.content .screenshot'));
...
const compareImages = async (page, baselineUrl, latestUrl, diffOutPath, serverUrl) => {
  ...
  const result = await page.evaluate(async (bUrl, lUrl) => {
    ...
    const sizeMismatch = baseline.naturalWidth !== latest.naturalWidth || baseline.naturalHeight !== latest.naturalHeight;
    ...
    for (let i = 0; i < a.data.length; i += 4) {
      ...
      const isDifferent = dr > perChannelThreshold || dg > perChannelThreshold || db > perChannelThreshold || da > perChannelThreshold;
      if (isDifferent) {
        mismatched++;
...
const main = async () => {
  ...
  try {
    const browser = await puppeteer.launch({
      defaultViewport: { width: SCREENSHOT_WIDTH, height: 800, deviceScaleFactor: DEVICE_PIXEL_RATIO },
      headless: 'new'
    });

    const examplePaths = await uniqueExamplePaths();
    console.log(`Found ${examplePaths.length} baseline example(s)`);
    ...
    for (const urlPath of examplePaths) {
      ...
      for (const mode of ['light', 'dark']) {
        ...
        try { await fs.access(baselinePng); baselineExists = true; } catch {}
        if (!baselineExists) {
          const rec = { route, mode, status: 'missing-baseline', baseline: baselinePng, latest: latestPng };
          records.push(rec);
          failures.push(rec);
        ...
        let res;
        try {
          res = await compareImages(comparePage, baselineUrl, latestUrl, diffPng, serverUrl);
        } catch (e) {
          res = { width: 0, height: 0, mismatched: 0, percentage: 1, sizeMismatch: false, error: String(e) };
        } finally {
  ...
  for (const r of records) {
    ...
    console.log(`${marker} ${String(r.mode || '-').padEnd(5)} ${r.route} diff=${pct}${r.sizeMismatch ? ' (size mismatch)' : ''}`);
  }
